...
export default function LoginPage() {
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md border border-slate-100">
      ...
      {activeTab === 'staff' && (
        <form onSubmit={handleStaffSubmit} className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <PinKeypad onKeyPress={handlePinPress} onDelete={handlePinDelete} onSubmit={() => handleStaffSubmit()} />
      ...
      {activeTab === 'admin' && (
        <form onSubmit={handleAdminSubmit} className="animate-in fade-in slide-in-from-left-4 duration-300">
      ...
      <div className="mt-8 bg-slate-50 border border-slate-100 rounded-lg p-3 flex gap-3">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          <span className="font-semibold text-slate-700">Notice:</span> Staff access is for authorized Somali Post
          personnel only. Admin access is for management and system oversight. Unauthorized access attempts may be
          logged.
        </p>
      </div>
    </div>
